...
export interface Field {
  ...
  income: number;
  /**
   * Тик последнего досрочного призыва.
   *
   * Флага «уже вызывал на этой волне» здесь недостаточно, и это выяснилось
   * дорогой ценой: команда обнуляет таймер, обработка волн в том же шаге
   * запускает волну и сбрасывает флаг — он не живёт ни одного тика. Призыв
   * сам переводит на новую волну, поэтому ограничивать надо время между
   * призывами, а не их число на волну.
   */
  lastRushTick: number;
